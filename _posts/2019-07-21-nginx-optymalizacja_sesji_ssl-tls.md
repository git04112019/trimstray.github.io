...
Zanim przeglądarka internetowa będzie mogła bezpiecznie wymieniać dane aplikacji, takie jak żądania i odpowiedzi protokołu HTTP, z serwerem sieciowym, musi najpierw ustalić parametry kryptograficzne bezpiecznej sesji. Stąd optymalizacja sesji SSL/TLS może być ciekawym punktem poprawiającym wydajność, jednak, według mnie i tak jedną z najistotniejszych rzeczy jest dostarczanie treści w taki sposób, aby zapewnić minimalną ilości danych, które będzie musiał pobrać klient. Możemy zastosować usługę CDN, która może uprościć wszelkie optymalizacje (zerknij do artykułu [How does a CDN improve load times?](https://www.cloudflare.com/learning/cdn/performance/)). Wniosek z tego taki, że optymalizacja protokołu SSL/TLS nie powinna być jedyną i kluczową częścią poprawy wydajności aplikacji webowej. Pamiętaj też, że obsesja na punkcie wartości i skrupulatnego dostrajania parametrów opisanych w tym wpisie, jest zdecydowanie czymś przesadzonym, ponieważ wydajność połączenia zależy od wielu czynników.
...
Głównym problemem związanym z wydajnością uzgadniania TLS nie jest (jak mogłoby się wydawać) to, jak długo to trwa, ale kiedy ma miejsce podczas komunikacji między klientem a serwerem. Ponieważ uzgadnianie jest częścią tworzenia bezpiecznego połączenia, musi nastąpić przed wymianą jakichkolwiek danych, przez co wydłuża czas, w którym przeglądarka nie może nic innego zrobić, spowalniając wydajność aplikacji internetowej. Przeglądarka nie może zrobić nic innego, dopóki nie otrzyma tej początkowej strony HTML. Nie może pobierać równolegle innych zasobów, takich jak pliki CSS lub obrazy, ponieważ nie uzyskała tej początkowej informacji, która mówi jej o innych zasobach. Dzieje się tak w przypadku każdej odwiedzanej strony internetowej: przeglądarka jest blokowana, aby uzyskać tę pierwszą odpowiedź.
...
W tym przypadku musisz pamiętać, aby odpowiednio „obracać” kluczem tak, by zapewnić mechanizm automatycznego odnawiania, np. restartując serwer co jakiś czas, co jednak nie zawsze rozwiązuje problem (Mozilla zaleca regenerowanie kluczy co 24 h). Co więcej, należy rozważyć przeniesienie tych kluczy do pamięci (wykorzystując np. <span class="h-b">tmpfs</span>), jednak moim zdaniem rodzi to zbyt wiele komplikacji związanych z zarządzaniem. Niezależnie od tego, przed jego ustawieniem, polecam zapoznać się z [RFC 4086 - Randomness Requirements for Security](https://tools.ietf.org/html/rfc4086). Pamiętajmy także, że po zamianie, stary plik klucza powinien być całkowicie zniszczony.
...
Jeśli twoje serwery mają wystarczającą moc, możesz rozważyć całkowite wyłączenie identyfikatorów sesji i biletów sesji. Według mnie jest to nadal zalecane rozwiązanie, aby zapewnić tajemnicę przekazywania, ponieważ większość używanych serwerów HTTP (Apache, NGINX) nie obsługuje odpowiedniej rotacji tych parametrów. Z drugiej strony, pamiętajmy, że koszt wydajności związany z niewykorzystaniem wznawiania sesji TLS jest znaczny.
