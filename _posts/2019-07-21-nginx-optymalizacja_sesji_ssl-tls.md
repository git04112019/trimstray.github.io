...
W tym wpisie chciałbym pomówić o optymalizacji parametrów sesji SSL/TLS na przykładzie serwera NGINX. Według mnie jest to jeden z ciekawszych (a może nawet istotniejszych) kroków do poprawy ogólnych wrażeń użytkowników podczas korzystania z aplikacji internetowych, nieodłącznie wpływając na szybkość ich reakcji. Dotyczy to zwłaszcza aplikacji, które wymagają pełnego uzgadniania protokołu TLS dla każdego połączenia sieciowego, a które to potrafi wprowadzić pewne opóźnienia, wydłużając czasy odpowiedzi i w konsekwencji obniżając ogólną wydajność.

Jednak przed rozpoczęciem dalszej lektury nie mogę nie wspomnieć, że są inne, niezwykle ważne (jeśli nie ważniejsze) rzeczy poprawiające wydajność aplikacji webowej, tj. włączenie protokołu HTTP/2, zastosowanie mechanizmów pamięci podręcznej, stosowanie usługi CDN, która może uprościć wszelkie optymalizacje (zerknij do artykułu [How does a CDN improve load times?](https://www.cloudflare.com/learning/cdn/performance/)), optymalizacja połączeń do bazy czy po prostu optymalizacja kodu. Moim zdaniem, najistotniejsze jest jednak dostarczanie treści w taki sposób, aby zapewnić minimalną ilości danych, które będzie musiał pobrać klient.

Zanim przeglądarka internetowa będzie mogła bezpiecznie wymieniać dane aplikacji, takie jak żądania i odpowiedzi protokołu HTTP, z serwerem sieciowym, musi najpierw ustalić parametry kryptograficzne bezpiecznej sesji. Stąd optymalizacja sesji SSL/TLS może być ciekawym punktem poprawiającym wydajność. Wniosek z tego taki, że optymalizacja tego zestawu protokołów nie powinna być jedyną i kluczową częścią poprawy wydajności aplikacji webowej. Pamiętaj też, że obsesja na punkcie wartości i skrupulatnego dostrajania parametrów opisanych w tym wpisie, jest zdecydowanie czymś przesadzonym, ponieważ wydajność połączenia zależy od wielu czynników.

Jeżeli chodzi o temat tego wpisu, to tak naprawdę nie ma jednoznacznych odpowiedzi, które dotyczą ustawienia odpowiednich czy optymalnych wartości parametrów omawianych parametrów. Strojenie ich jest trudne, ponieważ ciężko jest uzyskać odpowiedź na pytania, **jakich wartości należy użyć, w przypadku n klientów** lub **jakie wartości są odpowiednie dla danego środowiska**. Aby jeszcze bardziej skomplikować sprawę, pamiętajmy, że obecnie najczęściej wykorzystywane protokoły, tj. TLSv1.2 i TLSv1.3 posiadają pewne różnice. Co więcej, nie ma jednego standardu i różne projekty dyktują różne ustawienia.
...
W tym wpisie rozmawiać będziemy jedynie o niektórych parametrach protokołu SSL/TLS, które możemy zmodyfikować z poziomu serwera NGINX — należy mieć jednak świadomość, że nie są to oczywiście jedyne możliwości poprawy (nie tylko wydajności, ale także bezpieczeństwa) tego zestawu protokołów.
