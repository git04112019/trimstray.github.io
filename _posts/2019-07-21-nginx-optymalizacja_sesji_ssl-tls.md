...
W tym wpisie chciałbym pomówić o optymalizacji parametrów sesji SSL/TLS (na przykładzie serwera NGINX), ponieważ według mnie jest to jeden z najprostszych kroków do poprawy ogólnych wrażeń użytkowników podczas korzystania z aplikacji uruchamianych w przeglądarkach internetowych. Pamiętaj jednak, że obsesja na punkcie wartości i skrupulatnego dostrajania parametrów opisanych w tym wpisie, jest zdecydowanie czymś przesadzonym, ponieważ najprawdopodobniej w większości przypadków uzyskamy ledwie delikatny wzrost wydajności, wręcz niezauważalny podczas codziennej pracy. Myślę jednak, że warto mieć świadomość, który parametr, za co odpowiada, dlaczego warto rozważyć jego modyfikację, jakie niesie konsekwencje taka operacja oraz ile i co możemy zyskać po takiej zmianie.
...
W przypadku wdrożenia protokołu HTTPS musimy liczyć się z pojawiającym się opóźnieniem. Dzieje się tak, ponieważ początkowe uzgadnianie TLS wymaga dwóch dodatkowych obiegów w obie strony przed ustanowieniem faktycznego połączenia, w porównaniu do jednego przejścia z wykorzystaniem niezaszyfrowanego protokołu HTTP. Uzgadnianie TLS ma wiele odmian i należy pamiętać, że dokładny narzut TLS zależy od różnych czynników, a znaczący na niego wpływ będzie mieć zmienny rozmiar większości wiadomości, a także różne wzorce ruchu.

Przypomnijmy sobie, jak wygląda cały proces uzgadniania dla typowego połączenia SSL/TLS:
...
Celem pamięci podręcznej sesji SSL/TLS po stronie serwera jest zmniejszenie użycia procesora oraz zwiększenie wydajność z punktu widzenia klientów, dzięki wyeliminowaniu konieczność ciągłej renegocjacji sesji — czyli przeprowadzania nowego (i czasochłonnego) uzgadniania SSL/TLS przy każdym żądaniu.
