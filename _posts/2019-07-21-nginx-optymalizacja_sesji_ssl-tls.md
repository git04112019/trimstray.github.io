...
Natomiast co w przypadku protokołów SSL/TLS? Tutaj optymalizacji może być kilka, a najbardziej zróżnicowaną częścią w przypadku tych protokołów są certyfikaty (jak już wspomniałem kilkukrotnie), ponieważ oprócz ich rozmiaru, pamiętajmy o ich liczbie (certyfikat serwera i wszystkie pośrednie certyfikaty wystawcy w łańcuchu certyfikatów, bez certyfikatu głównego). Z racji tego, że rozmiary certyfikatów różnią się w zależności od użytych parametrów i kluczy, przyjąłbym wcześniejszą wartość 1500 bajtów na certyfikat (certyfikaty z podpisem własnym mogą mieć znacznie mniejszy rozmiar).
