...
Zanim przeglądarka internetowa będzie mogła bezpiecznie wymieniać dane aplikacji z serwerem sieciowym, takie jak żądania i odpowiedzi protokołu HTTP, musi najpierw ustalić parametry kryptograficzne bezpiecznej sesji. Krok ten jest kluczowym i nieodłącznym elementem podczas zestawiania szyfrowanego połączenia, a co istotne, jednym z najdłużej wykonywanych podczas nawiązywania sesji między obiema stronami komunikacji.

Moim zdaniem, optymalizacja sesji SSL/TLS powinna być jednym z ważniejszych elementów poprawiających ogólne wrażenia użytkowników podczas korzystania z aplikacji internetowych, nieodłącznie wpływając na szybkość ich reakcji. Dotyczy to zwłaszcza aplikacji, które wymagają pełnego uzgadniania protokołu TLS dla każdego połączenia sieciowego, a które to potrafi wprowadzić niemałe opóźnienia, wydłużając czasy odpowiedzi i w konsekwencji obniżając ogólną wydajność.
