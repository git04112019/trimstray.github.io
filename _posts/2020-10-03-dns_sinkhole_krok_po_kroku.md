...
Sam proces rozwiązywania nazw (czyli zamiany nazwy na adres IP) obejmuje mechanizm podobny do znajdowania numeru telefonu na podstawie imienia i nazwiska w książce telefonicznej (nie wymyśliłem żadnej nowej analogii, więc posłużę się tą starą jak świat). DNS jest właśnie taką książką, dzięki której możliwe było, na podstawie nazwiska danej osoby, uzyskanie wyniku będącego numerem telefonu przypisanym do niej, w wyniku czego mogliśmy się z nią skontaktować. System DNS służy temu samemu celowi.
...
To, co wpisujemy w przeglądarce, nazywamy nazwą domenową (lub po prostu domeną). Każda taka nazwa składa się z co najmniej jednej etykiety. Etykiety są oddzielone znakiem <span class="h-b">.</span> i mogą zawierać maksymalnie 63 znaki. W pełni kwalifikowana nazwa domeny (ang. _FQDN - Fully Qualified Domain Name_), czyli pełna nazwa domeny dla określonego komputera lub hosta, może zawierać maksymalnie 255 znaków, łącznie ze znakiem kropki. Etykiety są konstruowane od prawej do lewej strony, gdzie etykieta po prawej stronie jest domeną najwyższego poziomu (ang. _TLD - Top Level Domain_) dla nazwy domeny. Na przykład mając domenę <span class="h-b">foo.bar.example.com</span> etykieta znajdująca się najbardziej po prawej stronie, tj. <span class="h-b">.com</span> będzie etykietą TLD.

Protokół DNS został dokładnie opisany w kilku dokumentach RFC, natomiast najnowsze z nich to [RFC 1034 - Domain Names - Concepts And Facilities](https://tools.ietf.org/html/rfc1034) oraz [RFC 1035 - Domain Names - Implementation And Specification](https://tools.ietf.org/html/rfc1035). Warto także zajrzeć do [RFC 2671 - Extension Mechanisms for DNS (EDNS0)](https://tools.ietf.org/html/rfc2671). Przeglądając je, znajdziesz w nich odnośniki to starszych wersji.

Rozmawiając o protokole DNS, to z istotniejszych informacji, które musimy wiedzieć, jest to, że może on korzystać z obu protokołów warstwy transportu i domyślnie używa portu docelowego o numerze 53. Gdy protokół DNS używa UDP jako transportu, ma możliwość obsługi retransmisji i sekwencjonowania UDP. Natomiast protokół TCP jest najczęściej wykorzystywany, gdy rozmiar żądania lub odpowiedzi jest większy niż pojedynczy pakiet, na przykład w przypadku odpowiedzi, które mają wiele rekordów, w przypadku odpowiedzi IPv6 lub większości odpowiedzi DNSSEC.

  > Wszystkie informacje przechowywane o domenach są zdefiniowane (przechowywane) w rekordach zasobów (ang. _RR - Resource Records_), które najczęściej są pogrupowane w strefy i przechowywane lokalnie na serwerze DNS.
